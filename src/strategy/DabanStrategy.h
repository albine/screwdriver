...
private:
    ...
    uint32_t activate_price_ = 0;     // 激活阈值价格
...
public:
    ...
    void on_tick(const MDStockStruct& stock) override {
        ...
        if (stock.mdtime < 93000000) return;

        // 入场阶段保底触发（逐笔行情开盘前可能延迟）
        if ((state_ == State::SLEEPING || state_ == State::ACTIVE) &&
            limit_up_price_ > 0 && !signal_triggered_) {
    ...
    void on_order(const MDOrderStruct& order, const FastOrderBook& book) override {
        ...
        // 过滤早盘集合竞价
        if (current_time < 93000000) return;

        // POSITIONED 阶段: 喂数据给炸板检测器
...
private:
    ...
    void emit_buy_signal(int32_t current_time, const FastOrderBook& book) {
        ...
        LOG_M_INFO("DabanStrategy -> POSITIONED: {} | monitoring for break", symbol);
    }
    ...
    void emit_buy_signal_from_tick(int32_t current_time) {
        ...
        LOG_M_INFO("DabanStrategy -> POSITIONED: {} | monitoring for break (backup)", symbol);
    }

    void emit_sell_signal(int32_t trigger_time) {
        if (state_ != State::POSITIONED) return;
    ...
    static std::string state_to_string(State state) {
        ...
        case State::POSITIONED:   return "POSITIONED";
